...
st.markdown(f"<h1 style='text-align: center;'>{city1} ⚔️💥⚔️ {city2}</h1>", unsafe_allow_html=True)
...
if selected == "Données générales":
    ...
    with tab1:
        ...
        with col1:
            ...
            create_kpi_box("Résidences principales", int(data1["P21_RP"]))
        with col2:
            create_kpi_box("Logements en 2021", int(data2["P21_LOG"]))
            create_kpi_box("Résidences principales", int(data2["P21_RP"]))
    with tab2:
